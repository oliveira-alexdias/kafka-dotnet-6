...
namespace WebAPI.A.BaseHostedService;
...
public class BaseHostedService : IHostedService
{
    ...
    private void StartConsumeTopics(CancellationToken cancellationToken)
    {
        ...
        consumer.Subscribe(ObjectOfValue.Constants.TopicB);

        while (!cancellationToken.IsCancellationRequested)
        {
            if (!IsTopicAvailable()) continue;
            var topic = consumer.Consume(cancellationToken);
            _logger.LogInformation("Received data on {topic}: {value}", ObjectOfValue.Constants.TopicB, topic.Message.Value);
        }
    ...
    private bool IsTopicAvailable()
    {
        ...
        return metadata.Topics.Any(t =>
            t.Topic.Equals(ObjectOfValue.Constants.TopicB, StringComparison.InvariantCultureIgnoreCase));
    }
